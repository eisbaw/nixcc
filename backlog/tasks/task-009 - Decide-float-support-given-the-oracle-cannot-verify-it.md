...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Decision recorded: full float support, integer-only subset, or deferred to a later wave
- [ ] #2 If float is in scope, the verification strategy is named, since the lcc oracle cannot serve -- e.g. differential execution against the emulator rather than IR diffing
- [ ] #3 If float is out of scope, the frontend rejects float declarations with a clear diagnostic rather than silently miscompiling them
<!-- AC:END -->
