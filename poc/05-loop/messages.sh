...
# threw" is all must-fail.nix can check by itself; a driver whose every
...
#
# The same file, and the same awk, as poc/04-assembler/messages.sh. It is
# copied rather than shared because it is eight lines of glue around one file
# path, and because run.sh's mutation stage has to be able to run a MUTATED
# copy of it -- a shared one would be mutated for every PoC at once.
#
